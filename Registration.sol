...
//This function is called from the AHSC to validate the user and also retrieve the Id and the Role


    function getUserRole (address _userAddress, string _userCertificate) returns (uint _userId, string _userRole)  private {
    	for (uint i=1; i<User.length; i++){
    		if (User[i].address == userAddress && User[i].certificate == _userCertificate){
    			uint _userId = i;
    			string _userRole = User[i].role; 
    		    return _userId, _userRole;
    		}
    		else 
    			return "the user is not registered or invalid certificate has been provided";
